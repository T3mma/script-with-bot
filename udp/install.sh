...
# Run as root
[[ "$(whoami)" != "root" ]] && {
  echo -e "\033[1;33m[\033[1;31mERROR\033[1;33m] \033[1;37m- \033[1;33M debes iniciar como root\033[0m"
...
source <(curl -sSL 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/module')
...
if [ "$(lsb_release -rs)" = "8*|9*|10*|11*|16.04*|18.04*" ]; then
  ...
  print_center -ama -e "\e[1m\e[33m${a94:-El script  NO es compatible con tu sistema operativo}\e[0m"
  ...
  # [+get files ⇣⇣⇣+]
  source <(curl -sSL 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/module') &>/dev/null
  wget -O /etc/UDPCustom/module 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/module' &>/dev/null
  chmod +x /etc/UDPCustom/module

  wget "https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/udp-custom-linux-amd64" -O /root/udp/udp-custom &>/dev/null
  # wget "https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/udp-custom-linux-amd64" -O /usr/bin/udp-request &>/dev/null
  chmod +x /root/udp/udp-custom
  # chmod +x /usr/bin/udp-request

  wget -O /etc/limiter.sh 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/limiter.sh'
  cp /etc/limiter.sh /etc/UDPCustom
  chmod +x /etc/limiter.sh
  chmod +x /etc/UDPCustom

  # [+udpgw+]
  wget -O /etc/udpgw 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/udpgw'
  mv /etc/udpgw /bin
  chmod +x /bin/udpgw

  # [+service+]
  wget -O /etc/udpgw.service 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/udpgw.service'
  wget -O /etc/udp-custom.service 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/udpgw.service'
  ...
  # [+config+]
  wget "https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/config.json" -O /root/udp/config.json &>/dev/null
  chmod +x /root/udp/config.json

  # [+menu+]
  wget -O /usr/bin/udp 'https://raw.githubusercontent.com/T3mma/script-with-bot/main/udp/udp'
  chmod +x /usr/bin/udp
  ...
  print_center -ama "${a103:-  PARA VER EL MENU tipee: \nudp\n}"
  msg -bar
  time_reboot 6
fi
